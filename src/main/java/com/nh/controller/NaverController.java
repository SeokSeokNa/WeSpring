...
import com.nh.dto.naver.NaverTokenDto;
import com.nh.dto.naver.NaverUserDto;
...
public class NaverController {
    ...
    @Value("#{social['naver.key']}") /*servlet-context.xml 파일에 util 옵션으로 properties 파일을 지정해야 읽기가능*/
    private String apiKey;

    @Value("#{social['naver.secret']}")/*servlet-context.xml 파일에 util 옵션으로 properties 파일을 지정해야 읽기가능*/
    private String secret;


    private static final String authUrl = "https://nid.naver.com";
    private static final String redirectURI = "http://localhost:8080/oauth/result/naver";
    private static final String scope = "profile_nickname , profile_image ,account_email";


    @GetMapping("/loginPage/naver")
    public String showLoginPage() {
        String loginUrl = authUrl + "/oauth2.0/authorize?client_id=" + apiKey + "&redirect_uri=" + redirectURI + "&response_type=code";
        return "redirect:"+loginUrl;
    } //showLoginPage


    //로그인 하기(인가코드 받아 토큰 추출)
    @GetMapping("/oauth/result/naver") //Redirect Uri에 해당하는 Url
    public String loginKakao(@RequestParam("code") String code, @RequestParam(required = false, name = "error") String error , HttpServletResponse response) {
        ...
        NaverTokenDto tokenDto = getAccessToken(code);
        System.out.println("access_token = " + tokenDto.getAccess_token());
        NaverUserDto userInfo = getUserInfo(tokenDto.getAccess_token());
        System.out.println("userInfo = " + userInfo);
        System.out.println(userInfo.getResponse().getName());
        System.out.println(userInfo.getResponse().getProfile_image());


        //토큰으로 정보 조회후 현재 내 회원테이블에 있으면 넘어가고 없으면 회원가입 시킨 후 로그인 바로 시켜야 할듯
        HashMap<String, String> map = new HashMap<String, String>();

        map.put("userId", userInfo.getResponse().getEmail()); //카카오로부터 넘어온 이메일
        //암호화로직처리
        ...
        if (findUser == null) {
            ...
            userDto.setUserId(userInfo.getResponse().getEmail());
            //암호화로직처리
            //암호화 로직을 통한 데이터를 dto에 넣기
            userDto.setUserPass("1111");
            userDto.setUserName(userInfo.getResponse().getName());
            String gender = (userInfo.getResponse().getGender()).equals("M") ? "남자" : "여자";
            userDto.setGender(gender);
            userDto.setProfileImg(userInfo.getResponse().getProfile_image());
    ...
    //토큰얻기
    private NaverTokenDto getAccessToken(String code) {
        ...
        // 2. 토큰 요청 url 만들기   ,https://nid.naver.com/oauth2.0/token
    ...
    public NaverUserDto getUserInfo(String accessToken) {
        ...
        // 2. 사용자 정보 요청 url 만들기   ,https://openapi.naver.com/v1/nid/me
