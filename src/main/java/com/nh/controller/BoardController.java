...
public class BoardController {
    ...
    public String getBoardList(Model model , @RequestParam(value = "currentPage" , defaultValue = "0") int currentPage) {
        int pageData = 5;//한 페이지당 보여질 데이터 개수 (이건 내가 그냥 막 정하면되용)
        ...
        model.addAttribute("boardList",boardService.boardList(currentPage*pageData)); //게시판 리스트(몇번째 글부터 조회할지 offset을 파라미터로 넘겨줘야함)
        return "boardList";
    }
}
