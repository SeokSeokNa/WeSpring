            ...
            <div class="panel-body">
                <form role="form" action="/board/post" method="post">
                    ...
                    <div style="margin-left:1px;">
                        ...
                        <c:choose>
                            <c:when test="${cookie.get('user_id').value == boardContents.id}">
                                <a class="btn btn-warning pull-right" onclick="deleteConfirm();">삭제</a>
                                <a class="btn btn-danger pull-right"
                                   href="/board/update?num=${boardContents.num}">수정</a>
                            </c:when>
                        </c:choose>
                    </div>
                    <br>
                            ...
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item">
                                    <div class="form-inline mb-2">
                                        ...
                                        <input type="text" class="form-control ml-2" placeholder="Enter yourId"
                                               id="replyId">
                                        <label for="replyPassword" class="ml-4"><i
                                                class="fa fa-unlock-alt fa-2x"></i></label>
                                        <input type="password" class="form-control ml-2" placeholder="Enter password"
                                               id="replyPassword">
                                    </div>
                                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
                                    <button type="button" class="btn btn-dark mt-3" onClick="javascript:addReply();">
                                        post reply
                                    </button>
                                </li>
                            </ul>
...
<script type="text/javascript">
    ...
    function deleteConfirm() {
        if (!confirm("삭제하시겠습니까?")) {
            ...
            goDelete("/board/delete.do?num=${boardContents.num}");
